...
-- ── 2. Backfill row_number for existing rows (per table_id, ordered by created_at, row_id) ──

UPDATE rows r
SET row_number = sub.rn
FROM (
    SELECT row_id,
           ROW_NUMBER() OVER (PARTITION BY table_id ORDER BY created_at, row_id) AS rn
    FROM rows
) sub
...
-- Cast valid UUID strings to UUID, set non-UUID values (e.g. email strings) to NULL
ALTER TABLE rows
