...
-- Merge display_id + name into workspace_name on workspaces, drop
-- workspace_info (redundant)

-- 1. Add workspace_name column (populate from existing name if present)
ALTER TABLE workspaces
ADD COLUMN IF NOT EXISTS workspace_name VARCHAR NOT NULL DEFAULT '';
DO $$
BEGIN
...
-- 3. Drop workspace_info table (was just a duplicate of
--    workspaces.display_id + name)
...
ALTER TABLE workspaces ADD CONSTRAINT uq_workspaces_workspace_name UNIQUE (
    workspace_name
);

-- 5. Index on workspace_name for fast case-insensitive lookup
CREATE INDEX IF NOT EXISTS ix_workspaces_workspace_name ON workspaces (
    LOWER(workspace_name)
);
