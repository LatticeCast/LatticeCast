-- upgrade
-- 0003_create_tables_schema.sql
-- Workspace-based schema: workspaces, tables (columns JSONB), rows
-- (row_data JSONB)
-- Users table: email is the unique identity across all login methods

CREATE TABLE IF NOT EXISTS users (
    -- email address (same user regardless of login method)
...
CREATE TABLE IF NOT EXISTS workspace_members (
    workspace_id VARCHAR NOT NULL REFERENCES workspaces (
        workspace_id
    ) ON DELETE CASCADE,
