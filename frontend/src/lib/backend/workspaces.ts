...
export async function fetchWorkspaces(): Promise<Workspace[]> {
	...
	const response = await fetch(`${BACKEND_URL}/api/workspaces`, { headers });
...
export async function fetchMembers(workspaceId: string): Promise<WorkspaceMember[]> {
	const headers = await getAuthHeaders();
	const response = await fetch(`${BACKEND_URL}/api/workspaces/${workspaceId}/members`, { headers });
...
export async function addMember(workspaceId: string, data: AddMember): Promise<WorkspaceMember> {
	const headers = await getAuthHeaders();
	const response = await fetch(`${BACKEND_URL}/api/workspaces/${workspaceId}/members`, {
		method: 'POST',
		headers,
		body: JSON.stringify(data)
	});
...
export async function removeMember(workspaceId: string, userId: string): Promise<void> {
	const headers = await getAuthHeaders();
	const response = await fetch(`${BACKEND_URL}/api/workspaces/${workspaceId}/members/${userId}`, {
		method: 'DELETE',
		headers
	});
