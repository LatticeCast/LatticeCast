...
async def put_doc_blob_cell(
    ...
    if not row:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Row not found")
    existing_metadata = row.row_data.get(column_id)
    filename = (
        existing_metadata.get("filename")
        if isinstance(existing_metadata, dict) and isinstance(existing_metadata.get("filename"), str)
