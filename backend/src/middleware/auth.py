...
async def get_current_user(
    ...
    result = await session.execute(select(User).where(User.id == email))
...
async def require_admin(
    ...
    """
    if user.role != "admin":
        logger.warn(f"Admin access denied for: {user.id}")
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Admin access required",
        )

    logger.debug(f"Admin access granted: {user.id}")
    return user
...
async def require_user(
    ...
    """
    if user.role != "user":
        logger.warn(f"User access denied for: {user.id} (role={user.role})")
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Active subscription required",
        )

    logger.debug(f"User access granted: {user.id}")
    return user
