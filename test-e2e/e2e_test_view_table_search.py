...
Run:
    ...
    docker compose exec test-e2e python3 /scripts/e2e_test_view_table_search.py [--snapshot]
"""
...
def main() -> None:
    ...
    for name in ("Name", "City"):
        ...
        if not col:
            fatal(f"column {name!r} not found in schema: {[c['name'] for c in schema.get('columns', [])]}")
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            snap(page, "search_FAIL_no_rows")
            fatal("First row not visible after activating view")

        snap(page, "search_01_initial")
        ...
        try:
            ...
            snap(page, "search_FAIL_no_input")
            fatal("Search input not visible")

        search_input.fill("tok")
        page.wait_for_timeout(500)

        snap(page, "search_02_after_tok")

        vis_tok = visible_row_ids(page, row_ids)
        expected_tok = {rid_alice, rid_carol}
        if set(vis_tok) != expected_tok:
            fatal(f"After search 'tok': expected {expected_tok}, got {set(vis_tok)}")
        ...
        try:
            ...
            snap(page, "search_FAIL_no_clear_btn")
            fatal("Search clear button not visible")
        clear_btn.click()
        page.wait_for_timeout(500)

        snap(page, "search_03_after_clear")

        vis_clear = visible_row_ids(page, row_ids)
        if len(vis_clear) != 4:
            fatal(f"Expected 4 rows after clear, got {len(vis_clear)}: {vis_clear}")
        ...
        snap(page, "search_04_after_bob")

        vis_bob = visible_row_ids(page, row_ids)
        if set(vis_bob) != {rid_bob}:
            fatal(f"After search 'bob': expected only Bob ({rid_bob}), got {set(vis_bob)}")
        ...
        try:
            ...
            snap(page, "search_FAIL_manual_clear")
            fatal("Rows did not reappear after manual clear")

        snap(page, "search_05_after_manual_clear")

        vis_manual = visible_row_ids(page, row_ids)
        if len(vis_manual) != 4:
            fatal(f"Expected 4 rows after manual clear, got {len(vis_manual)}: {vis_manual}")
        ...
        snap(page, "search_06_after_zzz")

        vis_zzz = visible_row_ids(page, row_ids)
        if len(vis_zzz) != 0:
            fatal(f"After search 'zzz': expected 0 rows, got {len(vis_zzz)}: {vis_zzz}")
        ...
        try:
            ...
            snap(page, "search_FAIL_rows_not_back")
            fatal("Rows did not reappear after page reload")

        snap(page, "search_07_final")

        vis_final = visible_row_ids(page, row_ids)
        if len(vis_final) != 4:
            fatal(f"Expected 4 rows after reload, got {len(vis_final)}: {vis_final}")
    ...
    print("\n=== PASSED — e2e_test_view_table_search ===")
