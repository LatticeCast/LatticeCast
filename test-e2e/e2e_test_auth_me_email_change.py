#!/usr/bin/env python3
"""task-53: e2e_test_auth_me_email_change — email rotate via login_session.
...
Usage:
    docker compose exec test-e2e python3 /scripts/e2e_test_auth_me_email_change.py [--snapshot]
"""
...
def run() -> None:
    # [0] Login and discover current email
    print("[0] Login")
    token = login(USER)
    ...
    if r.status_code != 200:
        fatal(f"PUT /me/email reset failed: {r.status_code} {r.text[:200]}")
    ...
    if r.status_code != 201:
        fatal(f"create conflict user: {r.status_code} {r.text[:200]}")
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            # [10] UI: verify email input still shows the attempted value (not saved)
    ...
    print("PASS: e2e_test_auth_me_email_change")
